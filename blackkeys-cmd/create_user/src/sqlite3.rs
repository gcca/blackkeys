    ...
    match target {
        ...
        WriteTarget::Synced {
            ...
            block_on(async move {
                ...
                let mut statement = connection.prepare(INSERT_SQL).await.map_err(|failure| {
                    scrub(&auth_token, error("preparing user insert", &failure))
                })?;
                statement
                    .execute((username, password_hash, email))
                    .await
                    .map_err(|failure| scrub(&auth_token, error("inserting user", &failure)))?;
                connection
                    .cacheflush()
