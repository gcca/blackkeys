...
class NotifyServerStartedTests(unittest.TestCase):
    def TestNotifiesThatTheServerStarted(self) -> None:
        with patch(
            "blackkeys.monitor.socket.gethostname", return_value="test-host"
        ), patch("blackkeys.monitor.sentry_sdk.capture_message") as capture:
            NotifyServerStarted()

        capture.assert_called_once_with(
            "blackkeys server started on test-host", level="info"
        )
